...
int main(void)
{
	...
	float windY = 0.0f;
	float windZ = 1.0f;
	size_t const numTriangleIDs = sizeof(triangleIDs) / sizeof(unsigned int);
	bool visibleIDs[numTriangleIDs] = { false };
	size_t lenVertices = sizeof(vertices) / sizeof(float);
	while (true) {
		std::cout << "Rendering with wind direction: (" << windX << ", " << windY << ", " << windZ << ")" << std::endl;
		int status =  BinaryRenderer(vertices,lenVertices,triangleIDs,numTriangleIDs,visibleIDs,numTriangleIDs,windX,windY,windZ);

		std::cout << "Visible Triangle IDs:" << std::endl;
		for (unsigned int i = 0; i < numTriangleIDs; ++i) {
			if (visibleIDs[i]) {
